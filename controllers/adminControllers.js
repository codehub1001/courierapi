    ...
    return res.status(200).json({
      ...
      data: {
        ...
        pending: {
          ...
          totalPendingSystemProfit: pendingSystemProfit,
        },
        charts: {
...
export const adminGetAllVendorsPaymentHistory = async (req, res) => {
  try {
    // Fetch all delivered orders to calculate platform revenue generated by vendors
    const paymentRecords = await prisma.delivery.findMany({
      where: {
        status: "DELIVERED", // FIXED: Removed "PAID" since it's not a valid DeliveryStatus
      },
      select: {
        id: true,
        trackingId: true,
          ...
          select: {
            ...
            user: {
              select: {
                fullName: true,
                email: true,
                // FIXED: Removed wallet query here since vendors do not have wallets in your schema
              },
            },
          },
        },
...
// ─────────────────────────────────────────────
// 10. ADMIN: GET ALL RIDERS WITHDRAWAL HISTORY
// ─────────────────────────────────────────────
export const adminGetAllRidersWithdrawalHistory = async (req, res) => {
  try {
    const withdrawals = await prisma.walletTransaction.findMany({
      where: {
        type: "DEBIT", // A debit from the wallet represents a withdrawal
        wallet: {
          user: {
            role: "RIDER", // Ensures we are only looking at riders
          },
        },
      },
      select: {
        id: true,
        amount: true,
        status: true, // PENDING, COMPLETED, or FAILED
        referenceId: true,
        description: true,
        createdAt: true,
        wallet: {
    ...
    // Calculate total money successfully withdrawn by all riders
    ...
    console.error("Error fetching rider withdrawal history:", error);
    return res.status(500).json({
      success: false,
